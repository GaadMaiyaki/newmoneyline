            ...
            >
              ...
              >
                <step-one
                  ...
                  @employee_business_owner="workType"
                ></step-one>
              </tab-content>
              ...
              >
                <step-two
                  ref="stepTwo"
                  :loggedInUser="loggedInUser"
                  :workType="employee_or_business_owner"
                  @on-validate="onStepValidate"
                ></step-two>
              </tab-content>
...
export default {
  ...
  methods: {
    ...
    onStepValidate(validated, model) {
      if (validated) {
        this.finalModel = { ...this.finalModel, ...model };
          this.$axios.post("/loans/progress", this.finalModel).then(res => {
              this.$store.dispatch("users/fetchLoanFormProgress", res.data.form_data);
          });
      ...
      checkFormProgress(){
          this.$axios.get("/loans/progress/user", this.finalModel).then(res => {
              if (res.data) {
                  this.$store.dispatch("users/fetchLoanFormProgress", res.data.form_data);
              }
          });
    ...
    onComplete() {
      ...
      if (
        this.finalModel
          .currently_servicing_a_loan_from_another_financial_company == "true"
      ) {
      ...
      //Dis able finish button
        document.querySelectorAll('button.wizard-btn').forEach(elem => {
            elem.disabled = true;
        });
      this.$axios.post("/loans", this.finalModel).then(res => {
          this.$store.dispatch("users/fetchLoanFormProgress", '');
